...
export const scrapeAndStoreProduct = async (productUrl: string) => {
    ...
    try {
        ...
        //returning the plain product id
        const productId = newProduct._id.toHexString();
...
export const getSimilarProducts = async (productId: string) => {
    try {
        ...
        const similarProducts = await Product.find({
            ...
            .limit(4)
            .sort({ updatedAt: -1 })
            .exec(); //Get the 3 most recently updated products
...
export const addUserEmailToProduct = async (
    ...
    try {
        ...
        const userAlreadyTracking = product.users.some(
